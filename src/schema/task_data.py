...
class TaskData(BaseModel):
    name: str | None = Field(
        description="任务的名称。", default=None, examples=["Check input safety"]
    )
    run_id: str = Field(
        description="任务运行的 ID，用于配对状态更新。",
        default="",
        examples=["847c6285-8fc9-4560-a83f-4e6285809254"],
    )
    state: Literal["new", "running", "complete"] | None = Field(
        description="给定任务实例的当前状态。",
        default=None,
        examples=["running"],
    )
    result: Literal["success", "error"] | None = Field(
        description="给定任务实例的结果。",
        default=None,
        examples=["running"],
    )
    data: dict[str, Any] = Field(
        description="任务生成的附加数据。",
        default={},
    )
